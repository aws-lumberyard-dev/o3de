...
namespace AZ
{
    namespace Render
    {
        static AZ::Name s_clothDataName = AZ::Name::FromStringLiteral("CLOTH_DATA", AZ::Interface<AZ::NameDictionary>::Get());
        namespace Internal
        {
        ...
        bool MeshComponentController::RequiresCloning(const Data::Asset<RPI::ModelAsset>& modelAsset)
        {
            ...
            for (const AZ::Data::Asset<AZ::RPI::ModelLodAsset>& lodAsset : lodAssets)
            {
                ...
                {
                    if (mesh.GetSemanticBufferAssetView(s_clothDataName) != nullptr)
                    {
                        return true;
                    }
                }
        ...
        void MeshComponentController::HandleModelChange(Data::Instance<RPI::Model> model)
        {
            ...
            if (model && modelAsset)
            {
                ...
                //MaterialConsumerNotificationBus::Event(entityId, &MaterialReceiverNotificationBus::Events::OnMaterialAssignmentSlotsChanged);
