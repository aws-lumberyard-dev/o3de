...
Test case ID : C24274016
Test Case Title : Check that local ray cast scene queries work with collision groups set in script canvas nodes
URL of the test case : https://testrail.agscollab.com/index.php?/cases/view/24274016
"""
...
class Tests():
    ...
    find_ray_cast_local_entities = ("Raycasts local entities found",                   "One or more Raycast local entities not found")
    find_script_entity           = ("Script entity found",                             "Script entity not found")
    # entity results
    ray_cast_local_unknown       = ("ray_cast_local_unknown successfully deactivated", "ray_cast_local_unknown failed to deactivate")
    ray_cast_local_all           = ("ray_cast_local_all successfully deactivated",     "ray_cast_local_all failed to deactivate")
    ray_cast_local_none          = ("ray_cast_local_none successfully deactivated",    "ray_cast_local_none failed to deactivate")
    ray_cast_local_empty         = ("ray_cast_local_empty successfully deactivated",   "ray_cast_local_empty failed to deactivate")
# fmt: on


class LogLines:
    """
    All the Local Raycasts are expected to hit the top row of entities except the one with 'none' collision group.
    When expected lines found in logs, values from below expected_lines will be marked as True.
    """

    expected_lines = [
        "RayCastLocal Unknown true",
        "RayCastLocal All true",
        "RayCastLocal None false",
        "RayCastLocal Empty true",
    ]


def C24274016_ScriptCanvas_RayCastLocal_CollisionGroup():
    """
    Summary:
     Verify that Local ray cast scene queries work with collision groups set in script canvas nodes
    ...
    Level Description:
     ...
     4 raycast entities that start activated but will get deactivated if expected script canvas output is attained.
        Named ray_cast_local_unknown, ray_cast_local_all, ray_cast_local_none and ray_cast_local_empty.
     1 entity named 'script' containing script canvas to run C24274016_scriptcanvas_raycasts_local.scriptcanvas.

    Expected Behavior:
     When entered into Gamemode , all the Local Raycasts will hit the top row of entities except the one with 'none'
    ...
    Test Steps:
     ...
     5) Enter game mode
     6) Verify if all Raycasts deactivated
    ...
    RAYCASTS = ["ray_cast_local_unknown", "ray_cast_local_all", "ray_cast_local_none", "ray_cast_local_empty"]
    ...
    helper.init_idle()
    # 1) Open level
    helper.open_level("Physics", "C24274016_ScriptCanvas_RayCastLocal_CollisionGroup")
    ...
    Report.result(Tests.find_ray_cast_local_entities, all(entity.id.isValid() for entity in raycasts))
...
if __name__ == "__main__":
    ...
    Report.start_test(C24274016_ScriptCanvas_RayCastLocal_CollisionGroup)
