...
namespace AZStd
{
    allocator::pointer allocator::allocate(size_type byteSize, align_type alignment)
    {
        return AZ::AllocatorInstance<AZ::SystemAllocator>::Get().allocate(byteSize, alignment);
    }

    void allocator::deallocate(pointer ptr, size_type byteSize, align_type alignment)
    {
