...
namespace UnitTest
{
    static void CheckAllocatorsForLeaks(bool leakExpected)
    {
        ...
        for (int i = numAllocators - 1; i >= 0; --i)
        {
            ...
            if (records)
            {
                ...
                std::cout << records->GetAllocatorName() << " has " << records->RequestedBytes()
                          << " bytes, has allocations: " << (hasAllocations ? "yes\n" : "no\n");
                anyAllocatorHasAllocations |= hasAllocations;
            }
        }
...
#endif // AZ_TRAIT_DISABLE_FAILED_ALLOCATOR_LEAK_DETECTION_TESTS
    {
        ...
        auto& allocator = AZ::AllocatorInstance<TypeParam>::Get();
        std::cout << allocator.GetRecords()->GetAllocatorName() << " has " << allocator.GetRecords()->RequestedBytes() << " bytes\n";
    }
