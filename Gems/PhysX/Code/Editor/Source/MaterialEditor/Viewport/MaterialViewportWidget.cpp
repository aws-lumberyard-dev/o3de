...
namespace PhysXMaterialEditor
{
        ...
        , m_toolId(toolId)
    {
        ...
        // Create the framework scene and use the new entity context
        auto sceneSystem = AzFramework::SceneSystemInterface::Get();
        AZ_Assert(sceneSystem, "Unable to retrieve scene system.");
        AZ::Outcome<AZStd::shared_ptr<AzFramework::Scene>, AZStd::string> createSceneOutcome = sceneSystem->CreateScene("PhysXMaterialViewportScene");
    ...
    MaterialViewportWidget::~MaterialViewportWidget()
    {
        ...
        bool removeSuccess = sceneSystem->RemoveScene("PhysXMaterialViewportScene");
        if (!removeSuccess)
        {
            AZ_Assert(false, "MaterialViewportScene should be removed.");
        }
