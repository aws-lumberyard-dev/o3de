...
FUNCTION_SIGNATURE_PATTERN: str = "([a-zA-Z0-9:<>&*, ]*)([\n\r\s]+)([a-zA-Z0-9:]*)\((.*)\)"
FUNCTION_NAME_GROUP_INDEX: int = 3
...
def format_function_name(raw_function: str) -> str:
    matches = re.match(FUNCTION_SIGNATURE_PATTERN, raw_function)
    return matches.group(FUNCTION_NAME_GROUP_INDEX).strip()
        ...
        if clean_line and not clean_line.startswith(('#', '//', '/*', '*', '*/')):
            if clean_line.startswith('namespace'):
                if len(namespace_stack) - block_count > 1:
                    ...
                    current_block_stack = current_block_stack[:-1]
                namespace_stack.append(clean_line.split(' ')[1]) # ex
                current_block_stack.append([])
            ...
            if re.match(FUNCTION_SIGNATURE_PATTERN, clean_line):
                current_block_stack[-1].append(format_function_name(clean_line))
            
            if len(namespace_stack) > block_count and current_block_stack[-1]:
                print(f"Find {len(current_block_stack[-1])} functions in namespace {namespace_stack[-1]}")
                global PARSED_HEADER_FILE_CONTENT
                PARSED_HEADER_FILE_CONTENT[format_namespace(namespace_stack)] = current_block_stack[-1]
                current_block_stack = current_block_stack[:-1]
                namespace_stack = namespace_stack[:-1]
