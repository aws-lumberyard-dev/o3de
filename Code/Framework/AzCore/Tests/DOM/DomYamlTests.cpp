...
{
    class DomYamlTests : public UnitTest::AllocatorsFixture
    {
    public:
        void SetUp() override
        {
        ...
        AZStd::string WriteToYaml()
        {
            AZStd::string buffer;
            YamlBackend backend;
            backend.WriteToBuffer(buffer, [this](Visitor& visitor)
                {
    ...
    TEST_F(DomYamlTests, EmptyArray)
    {
        m_document->SetArray();
        PerformSerializationChecks();
    }

    TEST_F(DomYamlTests, SimpleArray)
    {
        m_document->SetArray();
        for (int i = 0; i < 5; ++i)
        {
    ...
    TEST_F(DomYamlTests, NestedArrays)
    {
        m_document->SetArray();
        for (int j = 0; j < 7; ++j)
        {
    ...
    TEST_F(DomYamlTests, EmptyObject)
    {
        m_document->SetObject();
        PerformSerializationChecks();
    }

    TEST_F(DomYamlTests, SimpleObject)
    {
        m_document->SetObject();
        for (int i = 0; i < 5; ++i)
        {
    ...
    TEST_F(DomYamlTests, NestedObjects)
    {
        m_document->SetObject();
        for (int j = 0; j < 7; ++j)
        {
