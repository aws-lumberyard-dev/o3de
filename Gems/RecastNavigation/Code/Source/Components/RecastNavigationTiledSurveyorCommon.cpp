...
#include "RecastNavigationTiledSurveyorCommon.h"
...
namespace RecastNavigation
{
    ...
    void RecastNavigationTiledSurveyorCommon::CollectGeometryAsyncImpl(
        float tileSize,
        float borderSize,
            ...
            for (int y = 0; y < tilesAlongY; ++y)
            {
                for (int x = 0; x < tilesAlongX; ++x)
                {
                    ...
                    AZ::TaskToken token = m_taskGraph.AddTask(
                        m_taskDescriptor, [this, geometryData, debugDrawInputData]()
                        {
