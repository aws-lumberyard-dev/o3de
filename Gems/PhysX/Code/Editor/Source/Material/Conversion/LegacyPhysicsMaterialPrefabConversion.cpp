...
namespace PhysX
{
    ...
    AZ_CONSOLEFREEFUNC("ed_physxFixPrefabsWithPhysicsLegacyMaterials", FixPrefabsWithPhysicsLegacyMaterials, AZ::ConsoleFunctorFlags::Null,
    ...
    /*
    PhysicsLegacy::MaterialId GetLegacyPhysicsMaterialIdFromComponent(const AzToolsFramework::Prefab::PrefabDomValue& prefabComponent)
    {
    ...
    */
    ...
    bool PrefabHasLegacyMaterialId(AzToolsFramework::Prefab::PrefabDom& prefabDom)
    {
        ...
        {
            for (auto* component : GetPrefabComponents<EditorColliderComponent>(*entity))
            {
            ...
            for (auto* component : GetPrefabComponents<EditorShapeColliderComponent>(*entity))
            {
            }

            for (auto* component : GetPrefabComponents<EditorCharacterControllerComponent>(*entity))
            {
            }

            for (auto* component : GetPrefabComponents<EditorHeightfieldColliderComponent>(*entity))
            {
            }
        }
        ...
        [[maybe_unused]] const LegacyMaterialIdToNewAssetIdMap& legacyMaterialIdToNewAssetIdMap)
    {
        ...
        for (auto* entity : GetPrefabEntities(prefabWithLegacyMaterials.m_template->GetPrefabDom()))
        {
            for (auto* component : GetPrefabComponents<Blast::EditorBlastFamilyComponent>(*entity))
            {
                ...
                {
                    AZ_Warning("PhysXMaterialConversion", false, "Unable to find a blast material asset to replace legacy material id '%s' with.",
                ...
                AZ_TracePrintf("PhysXMaterialConversion", "Legacy material id '%s' will be replaced by blast material asset '%s'.\n",
                    legacyMaterialId.m_id.ToString<AZStd::string>().c_str(),
                ...
                {
                    AZ_Warning("PhysXMaterialConversion", false, "Unable to set material asset id value to %s' in prefab.",
                        newMaterialAssetId.ToString<AZStd::string>().c_str());
                    continue;
                }
