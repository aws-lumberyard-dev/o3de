...
namespace AzToolsFramework
{
    namespace Prefab
    {
        ...
        void PrefabOverridePublicHandler::RevertOverrides(AZ::EntityId entityId)
        {
            AZStd::pair<AZ::Dom::Path, LinkId> pathAndLinkIdPair = GetPathToEntityFromFocusedPrefab(entityId);
            if (!pathAndLinkIdPair.first.IsEmpty() && pathAndLinkIdPair.second != InvalidLinkId)
            {
                m_prefabOverrideHandler.RevertOverrides(pathAndLinkIdPair.first, pathAndLinkIdPair.second);
            }
        }
